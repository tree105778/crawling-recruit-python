...
def fetch_site_info(pagenum, retries=3):
    ...
    for attempt in range(retries):
        try :
            ...
            logging.error(f"Error fetching page {pagenum}: {e}, attempt {attempt + 1}")
            if attempt < retries - 1:
                sleep_time = attempt
                logging.info(f"Retrying in {sleep_time} seconds...")
                time.sleep(sleep_time)
            else:
                logging.error(f"Max retries reached for page {pagenum}.")
                return None
